...
% Oblong Lobe device. Code obtains velocity profile and finds the closest
...
for fileName = 1:N
    ...
    y_coord = cell2mat(solData(:,3)).*10^6-1271; % Adjusts to make bottom of main channel y = 0
    ...
    x_lobe = [2419.4, 2706.5:264:10098.5]'; % immediately down stream of lobe
    xend = [2600.8:263.9:9990, 10277]; % immediately upstream
    ...
    for ii = 1:length(x_lobe)
        yOutMaxIdx = find(y_coord >= y_rng(2)-5 & y_coord <= y_rng(2)+5 & ...
         x_coord >= x_lobe(ii)-20 & x_coord <= xend(ii)+20 & z_coord >= 26 & z_coord <= 35);
